...
bool SpeechRecognizer::startOnline(const Config &config, QString *errorMessage)
{
    ...
    if (config.type == Type::StreamingTransducer) {
        ...
        recognizerConfig.model_config.transducer.encoder =
            m_online.encoderPath.c_str();
        recognizerConfig.model_config.transducer.decoder =
            m_online.decoderPath.c_str();
        recognizerConfig.model_config.transducer.joiner =
            m_online.joinerPath.c_str();
    }
    else {
        recognizerConfig.model_config.paraformer.encoder =
            m_online.encoderPath.c_str();
        recognizerConfig.model_config.paraformer.decoder =
            m_online.decoderPath.c_str();
    }
...
bool SpeechRecognizer::startOffline(const Config &config, QString *errorMessage)
{
    ...
    if (config.type == Type::SenseVoice) {
        ...
        if (!fileExists(model, errorMessage) ||
            !fileExists(tokens, errorMessage))
        {
            return false;
        }
        ...
        recognizerConfig.model_config.sense_voice.model =
            m_offline.senseVoiceModel.c_str();
        recognizerConfig.model_config.sense_voice.language =
            config.senseVoiceLanguage.toUtf8().constData();
        recognizerConfig.model_config.sense_voice.use_itn =
        ...
        if (!fileExists(adaptor, errorMessage) ||
            !fileExists(llm, errorMessage) || !fileExists(embed, errorMessage))
        {
            return false;
        }
        ...
        recognizerConfig.model_config.funasr_nano.encoder_adaptor =
            m_offline.funasrAdaptor.c_str();
        recognizerConfig.model_config.funasr_nano.llm =
            m_offline.funasrLlm.c_str();
        recognizerConfig.model_config.funasr_nano.embedding =
            m_offline.funasrEmbed.c_str();
        m_offline.funasrTok = tok.toUtf8().toStdString();
        recognizerConfig.model_config.funasr_nano.tokenizer =
            m_offline.funasrTok.c_str();
        m_offline.funasrSysPrompt =
            config.funasrSystemPrompt.toUtf8().toStdString();
        m_offline.funasrUserPrompt =
            config.funasrUserPrompt.toUtf8().toStdString();
        recognizerConfig.model_config.funasr_nano.system_prompt =
            m_offline.funasrSysPrompt.c_str();
        recognizerConfig.model_config.funasr_nano.user_prompt =
            m_offline.funasrUserPrompt.c_str();
        recognizerConfig.model_config.funasr_nano.max_new_tokens =
            config.funasrMaxNewTokens;
        recognizerConfig.model_config.funasr_nano.temperature =
        ...
        if (!fileExists(frontend, errorMessage) ||
            !fileExists(enc, errorMessage) || !fileExists(dec, errorMessage))
        {
            return false;
        }
        ...
        recognizerConfig.model_config.qwen3_asr.conv_frontend =
            m_offline.qwen3Frontend.c_str();
        recognizerConfig.model_config.qwen3_asr.encoder =
            m_offline.qwen3Encoder.c_str();
        recognizerConfig.model_config.qwen3_asr.decoder =
            m_offline.qwen3Decoder.c_str();
        m_offline.qwen3TokDir = config.qwen3TokenizerDir.toUtf8().toStdString();
        recognizerConfig.model_config.qwen3_asr.tokenizer =
            m_offline.qwen3TokDir.c_str();
        recognizerConfig.model_config.qwen3_asr.max_total_len =
            config.qwen3MaxTotalLen;
        recognizerConfig.model_config.qwen3_asr.max_new_tokens =
            config.qwen3MaxNewTokens;
        recognizerConfig.model_config.qwen3_asr.temperature =
            config.qwen3Temperature;
                                         ...
                                         int sampleRate, int channelCount)
{
    ...
    SherpaOnnxOnlineStreamAcceptWaveform(m_online.stream, sampleRate,
                                         samples.data(),
...
void SpeechRecognizer::publishOnlineResult(bool isFinal)
{
    ...
    if (!isFinal && text == m_online.lastText) {
        return;
    }
