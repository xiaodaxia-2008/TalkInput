...
void usage()
{
    std::cout << "Usage: TalkInputBenchmark --type <type> --dir <modelDir>"
              ...
              << "  moonshine_v2     (offline Moonshine v2, merged decoder)" << std::endl
...
BenchConfig parseArgs(const QStringList &args)
{
    ...
    if (c.modelDir.isEmpty() || c.audioPath.isEmpty()) {
        std::cerr << "Missing required arguments: --dir and --audio" << std::endl;
        std::exit(1);
    }
...
bool requireFile(const QString &path)
{
    if (QFileInfo::exists(path) && QFileInfo(path).isFile())
        return true;
    std::cerr << "Missing: " << QDir::toNativeSeparators(path).toStdString() << std::endl;
    return false;
}
...
QByteArray decodeAudioToPcm(const QString &audioPath)
{
    ...
    if (!ffmpeg.waitForStarted()) {
        std::cerr << "ffmpeg start failed: " << ffmpeg.errorString().toStdString() << std::endl;
        return {};
    }
    ...
    if (ffmpeg.exitStatus() != QProcess::NormalExit || ffmpeg.exitCode() != 0) {
        std::cerr << "ffmpeg error: " << err.trimmed().toStdString() << std::endl;
        return {};
    }
...
void setupSenseVoice(const BenchConfig &cfg, OfflineSetup &s)
{
    ...
    s.conf.model_config.sense_voice.model   = s.pool.store(modelFile(cfg.modelDir, "model.int8.onnx"));
...
{
    s.conf.model_config.fire_red_asr_ctc.model = s.pool.store(modelFile(cfg.modelDir, "model.int8.onnx"));
}

void setupFireRedAsrAed(const BenchConfig &cfg, OfflineSetup &s)
{
    s.conf.model_config.fire_red_asr.encoder = s.pool.store(modelFile(cfg.modelDir, "encoder.int8.onnx"));
    s.conf.model_config.fire_red_asr.decoder = s.pool.store(modelFile(cfg.modelDir, "decoder.int8.onnx"));
}
...
void setupMoonshineV2(const BenchConfig &cfg, OfflineSetup &s)
{
    ...
    s.conf.model_config.moonshine.encoder        = s.pool.store(modelFile(cfg.modelDir, "encoder_model.ort"));
    s.conf.model_config.moonshine.merged_decoder = s.pool.store(modelFile(cfg.modelDir, "decoder_model_merged.ort"));
}

void setupFunasrNano(const BenchConfig &cfg, OfflineSetup &s)
{
    s.conf.model_config.funasr_nano.encoder_adaptor = s.pool.store(modelFile(cfg.modelDir, "encoder_adaptor.int8.onnx"));
    s.conf.model_config.funasr_nano.llm             = s.pool.store(modelFile(cfg.modelDir, "llm.int8.onnx"));
    s.conf.model_config.funasr_nano.embedding       = s.pool.store(modelFile(cfg.modelDir, "embedding.int8.onnx"));
    s.conf.model_config.funasr_nano.tokenizer       = s.pool.store(modelFile(cfg.modelDir, "Qwen3-0.6B"));
    s.conf.model_config.funasr_nano.system_prompt   = "You are a helpful assistant.";
...
{
    s.conf.model_config.qwen3_asr.conv_frontend = s.pool.store(modelFile(cfg.modelDir, "conv_frontend.onnx"));
    s.conf.model_config.qwen3_asr.encoder       = s.pool.store(modelFile(cfg.modelDir, "encoder.int8.onnx"));
    s.conf.model_config.qwen3_asr.decoder       = s.pool.store(modelFile(cfg.modelDir, "decoder.int8.onnx"));
    s.conf.model_config.qwen3_asr.tokenizer     = s.pool.store(modelFile(cfg.modelDir, "tokenizer"));
...
QString runOnlineTransducer(const BenchConfig &cfg, double *elapsedSec)
{
    ...
    if (!rec) { std::cerr << "Failed to create online recognizer" << std::endl; return {}; }

    const SherpaOnnxOnlineStream *stream = SherpaOnnxCreateOnlineStream(rec);
    if (!stream) { SherpaOnnxDestroyOnlineRecognizer(rec); return {}; }

    QByteArray pcm = decodeAudioToPcm(cfg.audioPath);
    if (pcm.isEmpty()) { SherpaOnnxDestroyOnlineStream(stream); SherpaOnnxDestroyOnlineRecognizer(rec); return {}; }
    ...
    for (size_t off = 0; off < samples.size(); off += chunk) {
        int cnt = static_cast<int>(std::min(static_cast<size_t>(chunk), samples.size() - off));
        if (cnt == 0) break;
        SherpaOnnxOnlineStreamAcceptWaveform(stream, sampleRate, samples.data() + off, cnt);
        ...
        if (res) {
            QString partial = QString::fromUtf8(res->text ? res->text : "").trimmed();
        ...
        if (SherpaOnnxOnlineStreamIsEndpoint(rec, stream)) {
            const SherpaOnnxOnlineRecognizerResult *r2 =
                SherpaOnnxGetOnlineStreamResult(rec, stream);
            if (r2) {
                QString fin = QString::fromUtf8(r2->text ? r2->text : "").trimmed();
                if (!fin.isEmpty()) segments.append(fin);
                   ...
                   double *elapsedSec, double *loadSecOut = nullptr)
{
    ...
    if (!cfg.hotwordsPath.isEmpty() && requireFile(cfg.hotwordsPath)) {
        ...
        if (f.open(QIODevice::ReadOnly | QIODevice::Text)) {
            ...
            if (!hw.isEmpty()) {
                // Qwen3-ASR uses inline comma-separated hotwords (greedy_search)
                // Others use hotwords_file (requires modified_beam_search)
                if (cfg.type == "qwen3_asr") {
                    // Qwen3-ASR uses comma-separated hotwords instead of newlines
                    s.conf.model_config.qwen3_asr.hotwords = s.pool.store(
                        QStringList(hw.split('\n', Qt::SkipEmptyParts)).join(','));
                } else {
                    s.hwFile.setFileTemplate(QDir::temp().filePath("hw_XXXXXX.txt"));
                    if (s.hwFile.open()) {
                        s.hwFile.write(hw.toUtf8());
                        s.hwFile.flush();
                        s.conf.hotwords_file  = s.pool.store(QDir::toNativeSeparators(s.hwFile.fileName()));
    ...
    if (!rec) { std::cerr << "Failed to create offline recognizer (load=" << loadSec << "s)" << std::endl; return {}; }
    if (loadSecOut) *loadSecOut = loadSec;

    QByteArray pcm = decodeAudioToPcm(cfg.audioPath);
    if (pcm.isEmpty()) { SherpaOnnxDestroyOfflineRecognizer(rec); return {}; }
    ...
    for (size_t off = 0; off < samples.size(); off += chunkSamples) {
        int cnt = static_cast<int>(std::min(static_cast<size_t>(chunkSamples), samples.size() - off));
        if (cnt <= 0) continue;
        ...
        SherpaOnnxAcceptWaveformOffline(stream, sampleRate, samples.data() + off, cnt);
        SherpaOnnxDecodeOfflineStream(rec, stream);

        const SherpaOnnxOfflineRecognizerResult *res =
            SherpaOnnxGetOfflineStreamResult(stream);
        if (res) {
            QString text = QString::fromUtf8(res->text ? res->text : "").trimmed();
...
void compareWithGroundTruth(const QString &hypothesis, const QString &gtPath)
{
    QFile f(gtPath);
    if (!f.open(QIODevice::ReadOnly | QIODevice::Text)) {
        std::cerr << "Cannot open ground truth file: " << gtPath.toStdString() << std::endl;
        return;
    }
...
int main(int argc, char *argv[])
{
    ...
    if (cfg.type == "transducer")
        ...
        std::cerr << "Unknown model type: " << cfg.type.toStdString() << std::endl;
        usage();
        return 1;
    }
    ...
    std::cout << "  Model dir : " << QDir::toNativeSeparators(cfg.modelDir).toStdString() << std::endl;
    std::cout << "  Audio     : " << QDir::toNativeSeparators(cfg.audioPath).toStdString() << std::endl;
    if (cfg.type == "transducer" || cfg.type == "paraformer") {
        ...
        std::cout << "  Total time: " << (loadSec + elapsedSec) << " s" << std::endl;
    }
