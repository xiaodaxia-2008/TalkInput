...
std::vector<RawVadSegment> extractVadSegments(const std::vector<float> &floats,
                                              int sampleRate)
{
    const QString vadPath = QDir::current().filePath(
...
std::vector<MergedChunk>
mergeSegments(const std::vector<RawVadSegment> &rawSegs, int sampleRate,
              int targetMinSec = 10, int targetMaxSec = 18)
{
    ...
    for (size_t i = 1; i < rawSegs.size(); ++i) {
        ...
        // If current accumulated length is already >= targetMinSec and adding
        // next exceeds targetMaxSec, finish current chunk at natural pause
        if ((curEnd - curStart >= targetMinSamples) &&
            (combinedLength > targetMaxSamples))
        {
                          ...
                          int paddingMs)
{
    ...
    SPDLOG_INFO("  [SmartMerged VAD] Raw segments: {}, Merged into {} chunks "
                "(Target: 10~18s), Padding: {}ms",
    ...
    for (size_t i = 0; i < mergedChunks.size(); ++i) {
        ...
        const int paddedEnd =
            std::min(totalSamples, chunk.start + chunk.count + padSamples);
        ...
        for (int k = 0; k < paddedCount; ++k) {
            const float s = std::clamp(
                floats[static_cast<size_t>(paddedStart + k)], -1.0f, 1.0f);
...
void testEvaluation(const QString &presetId, const QString &audioPath)
{
    ...
    SPDLOG_INFO(
        "================================================================");
    SPDLOG_INFO("Testing: Model = {} ({}) | Audio = {}", preset.name, preset.id,
    ...
    QString res1 = runBaseline(preset, decoded->pcm16, decoded->sampleRate,
                               decoded->channels);
    auto t1_dur = std::chrono::duration_cast<std::chrono::milliseconds>(
                      std::chrono::steady_clock::now() - t1_start)
                      .count();
    SPDLOG_INFO("--- [1. Current Offline Recognizer Path] ({} ms) ---\n{}\n",
    ...
    auto t2_dur = std::chrono::duration_cast<std::chrono::milliseconds>(
                      std::chrono::steady_clock::now() - t2_start)
                      .count();
    SPDLOG_INFO(
        "--- [2. Smart Merged VAD (10~18s Chunk, 0ms Pad)] ({} ms) ---\n{}\n",
    ...
    auto t3_dur = std::chrono::duration_cast<std::chrono::milliseconds>(
    ...
    auto t4_dur = std::chrono::duration_cast<std::chrono::milliseconds>(
...
int main(int argc, char *argv[])
{
    ...
    // 1. SenseVoice
    testEvaluation(
        QStringLiteral("sense-voice-zh-en-ja-ko-yue-int8-2024-07-17"),
        fiberartPath);
    testEvaluation(
        QStringLiteral("sense-voice-zh-en-ja-ko-yue-int8-2024-07-17"),
        meetingPath);
